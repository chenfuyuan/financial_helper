...
async def test_full_syncs_all_stocks():
    ...
    result = await SyncFinanceIndicatorFullHandler(basic_repo, fi_repo, gateway, _uow()).handle(
        SyncFinanceIndicatorFull()
    )
...
async def test_by_stock_returns_count():
    ...
    result = await SyncFinanceIndicatorByStockHandler(fi_repo, gateway, _uow()).handle(
        SyncFinanceIndicatorByStock(ts_code="000001.SZ")
    )
...
async def test_increment_uses_latest_end_date():
    ...
    await SyncFinanceIndicatorIncrementHandler(basic_repo, fi_repo, gateway, _uow()).handle(
        SyncFinanceIndicatorIncrement()
    )
