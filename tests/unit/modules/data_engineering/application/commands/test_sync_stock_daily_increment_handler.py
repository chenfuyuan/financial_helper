...
async def test_retry_failures_success_and_failure(
    ...
    failure1 = StockDailySyncFailure(
        id=1, source=DataSource.TUSHARE, third_code="000001.SZ",
        start_date=date(2026, 1, 1), end_date=date(2026, 1, 10),
        error_message="err", failed_at=None, retry_count=0, resolved=False
    )
    failure2 = StockDailySyncFailure(
        id=2, source=DataSource.TUSHARE, third_code="000002.SZ",
        start_date=date(2026, 1, 1), end_date=date(2026, 1, 10),
        error_message="err", failed_at=None, retry_count=0, resolved=False
    )
    ...
    # 第一个成功，第二个失败
    mock_gateway.fetch_stock_daily.side_effect = [
        ["mock_record"],
        Exception("still failing")
    ]
    
    handler = RetryStockDailySyncFailuresHandler(
        gateway=mock_gateway, daily_repo=mock_daily_repo,
        failure_repo=mock_failure_repo, uow=mock_uow
    )
